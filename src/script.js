...
bush2.position.set(1.2,0.1,2.5)
 ...
 */
// Ambient light
const ambientLight = new THREE.AmbientLight('#b9d9ff', 0.12)
...
const ghost4 = new THREE.PointLight('#78caff', 2, 3)
scene.add(ghost1)
scene.add(ghost2)
scene.add(ghost3)
scene.add(ghost4)
...
// Fog
const fog = new THREE.Fog('#262837', 3, 17)
scene.fog = fog
...
const tick = () =>
{
    ...
    const ghost1Angle = elapsedTime * 0.55
    ghost1.position.set(
        Math.cos(ghost1Angle) * 4.5,
    ...
    ghost3.position.set(
        ...
        Math.sin(ghost3Angle) * 6.4
    )

    const ghost4Angle = -elapsedTime * 0.5
    ghost4.position.set(
        Math.cos(ghost4Angle) * (7 + Math.sin(elapsedTime) * 0.32),
        Math.sin(ghost4Angle) * (3 * Math.sin(elapsedTime) * 0.5),
        Math.sin(ghost4Angle) * 8
    )
